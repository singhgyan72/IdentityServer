...
public class SeedUserData
{
        ...
        using (var serviceProvider = services.BuildServiceProvider())
        {
                ...
                .GetRequiredService<IServiceScopeFactory>().CreateScope())
            {
                CreateUser(scope, "John", "Doe", "John Doe's Boulevard 323", "USA",
                    "97a3aa4a-7a89-47f3-9814-74497fb92ccb", "Password",
        ...
        string address, string country, string id, string password, string role, string email)
    {
        ...
        if (user == null)
        {
            ...
            // We need to confirm the email to be able to login
            result = userMgr.ConfirmEmailAsync(user, userMgr.GenerateEmailConfirmationTokenAsync(user).Result).Result;
            CheckResult(result);
        }
    }
